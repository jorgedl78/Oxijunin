...
Private Sub cmdAnular_Click()
    ...
    cn.Open
    cn.Execute ("UPDATE Recibos SET Anulado='Anulado' WHERE idRecibo=" & grRecibos.TextMatrix(grRecibos.Row, 0))
    ...
    cn.Execute ("AgregarCuentaCorriente '" & Format(Date, "yyyy/mm/dd") & "','Anulación Recibo " & grRecibos.TextMatrix(grRecibos.Row, 2) & "'," & Replace(grRecibos.TextMatrix(grRecibos.Row, 3), ",", ".") & ",0.00," & idCliente & ",'Anu'," & grRecibos.TextMatrix(grRecibos.Row, 0))
    cn.Close
...
Private Sub Consultar()
    ...
    Set rs = cn.Execute("SELECT idRecibo, Fecha, Numero, Importe, IsNull(Anulado,'') as Anulado, Detalle  from Recibos WHERE idCliente BETWEEN " & idCliente & " and " & idCliente & " AND Fecha BETWEEN '" & CalendarDesde.Value & "' AND '" & CalendarHasta.Value & "'")
    ...
    Do While rs.EOF = False
        .Rows = .Rows + 1
        .TextMatrix(.Rows - 1, 0) = rs!idRecibo

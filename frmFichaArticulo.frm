...
Begin VB.Form frmFichaCliente 
   ...
   Begin VB.Frame frPrecio 
      ...
      Begin VB.OptionButton optPublico 
         ...
         Width           =   1575
      End
      Begin VB.OptionButton optRevendedor 
         Caption         =   "Revendedor"
         Height          =   255
         Left            =   240
         TabIndex        =   21
...
Private Sub cmdGuardar_Click()
    ...
    cn.Open
    If optRevendedor.Value = True Then
        PrecioRevendedor = 1
    Else
        PrecioRevendedor = 0
    End If
    If Estado = "Modificando" Then
        cn.Execute ("GuardarCliente '" & txtNombre & "','" & txtDomicilio & "','" & txtTelefonos & "','" & txtMail & "'," & cmCategorias.ItemData(cmCategorias.ListIndex) & "," & Val(txtCodigo) & "," & cmbTipo.ItemData(cmbTipo.ListIndex) & "," & Val(txtNumeroDocumento) & ",'" & txtLocalidad & "'," & PrecioRevendedor)
    Else
        cn.Execute ("AgregaCliente '" & txtNombre & "','" & txtDomicilio & "','" & txtTelefonos & "','" & txtMail & "'," & cmCategorias.ItemData(cmCategorias.ListIndex) & "," & cmbTipo.ItemData(cmbTipo.ListIndex) & "," & Val(txtNumeroDocumento) & ",'" & txtLocalidad & "'," & PrecioRevendedor)
    End If
    cn.Close
    Saltar = 0
    Unload Me
...
Private Sub Form_Load()
    ...
    If Estado = "Modificando" Then
        ...
        If rs.EOF = False Then
            ...
            Next I
            If rs!PrecioRevendedor = 1 Then
                optRevendedor.Value = True
            Else
                optPublico.Value = True
            End If
        End If
    Else
        cmbTipo.ListIndex = 0
